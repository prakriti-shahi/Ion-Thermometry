...
# ╔═╡ 1633bbcc-b8f1-11f0-9637-734a461905a3
begin
	using PlutoUI, Plots
	kB = 1.380649*10^-23
...
# ╔═╡ 2f657f11-3dd8-48b0-baeb-1e220f89e739
begin
...
# ╔═╡ f27f9277-0a52-45a5-a350-3d7b3bafa050
@bind secular_frequency Slider(0.1:0.01:3.0, 1, true)

# ╔═╡ 3746ce59-9660-40d9-a6fb-f4a24453a6bf
# T(mK) slider
@bind FoV Slider(0.001:0.001:5, default=1, show_value=true)

# ╔═╡ 97c8f8b8-2bbb-4f0a-add5-e232a08b278b
begin
	# Compute n and probabilities
	T = range(10^-6,FoV,300)
	nbar = TempToAvgN(kB.*T.*10^-3, hbar*2*pi*secular_frequency*MHz)
	plot(T, nbar,
		xlabel="T (mK)",
		ylabel="n̄",
		label="ν = $(secular_frequency) MHz",
		title="Yb⁺  n̄ vs T for secular frequency = $(secular_frequency) MHz",
		linewidth=2,
		legend=:topright)
end
