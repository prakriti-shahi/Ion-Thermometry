...
# ╔═╡ a6bdf91a-be4a-11f0-8f20-2d49c4922572
begin
...
# ╔═╡ 4db3cabe-1fc1-46b7-83b3-a687c06ce78e
begin
	using GLMakie
	GLMakie.activate!()
end

# ╔═╡ 9211e5c9-b8eb-49d5-8545-da8708c85352
# Ωₙₘ(η)
function Ω(n::Int, m::Int, η::Float64)
    ...
    elseif η == 0
        ...
        return prefactor * ratio * ClassicalOrthogonalPolynomials.laguerrel(min(n, m), abs(n - m), η^2)
    end
end

# ╔═╡ 0adfe78f-5276-438d-a956-748556860f70
@bind eta PlutoUI.Slider(0:0.01:5.0, 0.1, true)

# ╔═╡ e22b71ce-3984-49e5-994c-c10d96243ee3
@bind FoV PlutoUI.Slider(1:1:100, 10, true)

# ╔═╡ 6f2dc8c1-a34f-4fa0-af89-a059b4dfac4e
begin
    # using GLMakie
    # GLMakie.activate!()
    ...
    ax = Axis3(
        fig[1, 1],
        title = "Debye–Waller Factor  |⟨n|exp[iη(â + â†)]|m⟩|   (η = $(round(eta, digits=2)))",
        xlabel = "n",
        ylabel = "m",
        zlabel = "|Ωₙₘ|",
    )

    gap = 0.15  # spacing between bars
    width = 0.8 - gap
    ...
    # Add light and grid adjustments
    ax.viewmode = :fit
    ax.azimuth = 40
    ax.elevation = 10
...
manifest_format = "2.0"
project_hash = "c0fe2c6b41c4bd8c4c1e8f4ea0fd448442bd30d5"
