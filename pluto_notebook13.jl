...
# ╔═╡ 6212a478-c3e0-11f0-a937-13d3e2d60594
begin
	using PlutoUI, Plots
	hbar = 1.054571817*10^-34
	Da = 1.66053906660*10^-27
	nm = 10^-9
	kHz = 10^3
	MHz = 10^6
	kB = 1.380649*10^-23
...
# ╔═╡ 9da537a9-3875-4250-b7bf-3c0f6f4e8ef4
begin

	using ClassicalOrthogonalPolynomials

	function TempToAvgN(T, ω)
		return 1.0 / (exp(hbar*ω / (kB*T)) - 1.0)
	end

	function η(λ, ω)
    	return (2π / λ) .* sqrt.(hbar ./ (2 * m .* ω))
	end
	...
	function Pth(n, avg)
	    if avg == 0
	        ...
	        return avg^n / (avg + 1)^(n + 1)
    	end
	end

	function Ω(n::Int, m::Int, η::Float64)
		k = min(n, m)
    	d = abs(n - m)
    	if n < 0 || m < 0
        	return 0.0
    	elseif η == 0
        	...
        	prefactor = exp(-η^2 / 2) * η^abs(n - m)
        	ratio = 1.0
    		for j in k+1:k+d
        		ratio /= sqrt(j)
    		end
        	return prefactor * ratio * laguerrel(min(n, m), abs(n - m), η^2)
    	end
	end
end

# ╔═╡ 4727a6e8-dd22-43a6-8f9e-491cb03f3163
function Pe(t, s::Int, avg::Float64, η::Float64, truncate::Int)
    total = 0.0
    for n in 0:truncate
        total += Pth(n, avg) * cos(2π * Ω(n + s, n, η) * t)
    end
    return 0.5 - 0.5 * total
end

# ╔═╡ 14c41c94-36e7-46bd-844e-77b559827150
@bind T Slider(0:1:2000, default=1650, show_value=true) # µK

# ╔═╡ 172888d8-5580-453c-a5fe-fa5d0f59e8c0
@bind nu Slider(0.01:0.01:3.0, default=1.0, show_value=true) #MHz

# ╔═╡ 234168d1-a179-46a8-b164-713aad0acf43
@bind FoV Slider(1:1:50, default=10, show_value=true)

# ╔═╡ 6bc765dd-305c-4cc2-8bfe-7acf4a7748cc
begin
	ω_rad = 2π * nu * MHz
    eta = η(λ, ω_rad)
    navg = TempToAvgN(T*1e-6, ω_rad)   # T in µK → K
	t = range(0, FoV, length=600)
	y_carrier = [Pe(tt,  0, navg, eta, 200) for tt in t]
    y_red     = [Pe(tt, -1, navg, eta, 200) for tt in t]
    y_blue    = [Pe(tt, +1, navg, eta, 200) for tt in t]
    y_ref     = [Pe(tt,  0, 0.0, 0.0, 0) for tt in t]
    ...
    plt = plot(
        ...
        xlabel="Ω t / 2π",
        ylabel="Pₑ",
        title="Flopping on the Carrier & 1st red/blue sidebands\n" *
    	"T = $(round(T; digits=2)) µK, ν = $(round(nu; digits=2)) MHz, nbar = $(round(navg; digits=2)), & eta = $(round(eta; digits=2))"
    	)

    plot!(plt, t, y_red,
        lw=3, color=:red,
        label="1ˢᵗ Red Sideband"
    )

    plot!(plt, t, y_blue,
        lw=3, color=:blue,
        label="1ˢᵗ Blue Sideband"
    )

    plot!(plt, t, y_ref,
        lw=2, linestyle=:dot, color=:gray,
        label="motion independent reference (η=0)"
    )

    plt
end

# ╔═╡ ca0cf343-6168-4f66-83bf-a835fd19f0ba
@bind Temp Slider(0:0.001:2000, 1600, true)

# ╔═╡ f8259210-6a95-4b6d-b27d-975cd2a496c5
begin
    freqs = [nu1, nu2, nu3]                     # three secular frequencies (MHz)
    labels = ["ν₁", "ν₂", "ν₃"]
    plots = []                              # array to store plots

    for (idx, nu_val) in enumerate(freqs)

        ω_rad = 2π * nu_val              # MHz → rad/s
        eta = η(λ, ω_rad)
        navg = TempToAvgN(Temp*1e-6, ω_rad)        # T µK → K
        ...
        p = plot(
            ...
            ylabel="Pₑ",
            title="Flopping on the Carrier & 1st red/blue sidebands\n" *
                  "T = $(round(Temp;digits=2)) µK,  $(labels[idx])=$(round(nu_val/MHz;digits=2)) MHz,  " *
                  "nbar=$(round(navg;digits=2)),  eta=$(round(eta;digits=3))"
        )

        plot!(p, t, y_red,  lw=3, color=:red,  label="1st Red Sideband")
        plot!(p, t, y_blue, lw=3, color=:blue, label="1st Blue Sideband")
        plot!(p, t, y_ref,  lw=2, linestyle=:dot, color=:gray,
            label="motion independent reference (η=0)")

        push!(plots, p)
    end

    plot(plots..., layout=(3,1), size=(600,1200))
end
...
[compat]
ClassicalOrthogonalPolynomials = "~0.15.10"
Plots = "~1.41.1"
PlutoUI = "~0.7.4"
"""
...
manifest_format = "2.0"
project_hash = "7ce7a2e8b3319a77f847cd7ff2165f693600feca"
...
git-tree-sha1 = "d02d10b9dd0f94f180f7d04e817dd8cd07ecb3a8"
...
git-tree-sha1 = "aafc249a1a7da83316e9f78a1985d1c4426f9233"
