...
# ╔═╡ 3b48abfa-c118-11f0-a656-dda6b9f44a5d
begin
	using PlutoUI, Plots
...
# ╔═╡ b400c00c-5de3-49a4-96b5-f928d99fc841
begin
...
# ╔═╡ f41ede59-f67e-4d5c-8b81-678105b00552
...
# ╔═╡ 6270eedd-ba63-4dff-ab3d-57b9310c2d80
@bind navg Slider(0.0:0.1:100.0, default=10.0, show_value=true)

# ╔═╡ d6dfce33-e473-4b38-abf9-5dfe3da56773
@bind η Slider(0:0.01:1.0, default=0.1, show_value=true)

# ╔═╡ 57d0d4d6-38ca-4fa8-a12e-1df5b375cd08
@bind s Select(["-2" => -2, "-1" => -1, "0" => 0, "1" => 1, "2" => 2], "0")

# ╔═╡ bd7a4baf-b8c3-49c7-8188-1603658ed87c
@bind FoV Slider(1:1:50, default=10, show_value=true)

# ╔═╡ b92b9510-31ab-42e6-801a-ee9f6e91d759
begin
    # convert Select output (label) -> integer sideband
    s_int = s isa Int ? s : parse(Int, s)

    # time array
    t = range(0, FoV, length=500)

    # main curve (coupled spin–motion)
    y1 = [Pe(tt, s_int, navg, η, 100) for tt in t]

    # reference curve (η = 0)
    y2 = [Pe(tt, 0, 0.0, 0.0, 0) for tt in t]

    plt = plot(
        t, y1,
        lw = 2.5,
        color = :steelblue,
        label = "coupled spin/motion\n(η = $(round(η, digits=2)))",
    )
    ...
    plot!(
        ...
        linestyle = :dot,
        color = :gray,
        label = "motion independent\nreference (η = 0)",
    )

    xlabel!("Ω t / 2π")
    ylabel!("Pₑ")
	ylims!(0.0,1.0)
    title!("Flopping on sideband $s_int for n̄ = $(round(navg, digits=1)) & η = $(round(η, digits=2))")

    plt
end


# ╔═╡ f214b7e6-329a-4d28-a79d-251b6a6e9826
begin
    ...
    y_carrier = [Pe(tt, carrier_s, navg, η, 100) for tt in t]
    y_red     = [Pe(tt, red_s,     navg, η, 100) for tt in t]
    y_blue    = [Pe(tt, blue_s,    navg, η, 100) for tt in t]
    y_ref     = [Pe(tt, 0,         0.0, 0.0, 0) for tt in t]   # η = 0 reference
    ...
    plot!(plt1, t, y_red,
        lw = 3,
        color = :red,
        label = "1st Red Sideband",
    )

    plot!(plt1, t, y_blue,
        lw = 3,
        color = :blue,
        label = "1st Blue Sideband",
    )
    ...
    xlabel!("Ω t / 2π")
    ylabel!("Pₑ")
    title!("Flopping on the Carrier & 1st sidebands for n̄=$(round(navg,digits=1)) & η=$(round(η,digits=2))")
