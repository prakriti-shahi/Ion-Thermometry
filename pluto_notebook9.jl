...
# ╔═╡ 592aab8e-c0df-11f0-83ae-1d59e0b3764d
begin
...
# ╔═╡ 40dd40ab-d20f-4743-ac4b-e1bebb61e4fa
begin
	using GLMakie
	GLMakie.activate!()
end

# ╔═╡ 7ec69112-707e-4c08-ad58-71256522004f
function η(λ, ω)
    return (2π / λ) * sqrt(hbar / (2 * m * ω))
end

# ╔═╡ 91976750-f7fc-4fae-9dfc-62b919c9f4e1
# Ωₙₘ(η)
function Ω(n::Int, m::Int, η::Float64)
    ...
    elseif η == 0
        ...
        ratio = sqrt(factorial(min(n, m)) / factorial(max(n, m)))
		L = ClassicalOrthogonalPolynomials.laguerrel(min(n, m), abs(n - m))
        return prefactor * ratio * L(η^2)
    end
end

# ╔═╡ 5f2c6620-297d-4e7d-b748-c3b7785c8e9f
@bind secular_frequency PlutoUI.Slider(0.01:0.001:3.0, 1, true)

# ╔═╡ 714de479-56bf-4a7a-896d-8a4e3342e420
@bind FoV PlutoUI.Slider(1:1:100, 10, true)

# ╔═╡ 6faba7c3-229c-48b1-8973-deb9b6b3b8f9
begin
    n_vals = 0:FoV
    m_vals = 0:FoV
    eta = η(411*nm, 2*pi*secular_frequency)
    ω = [Ω(n, m, eta) for n in n_vals, m in m_vals]

    fig = Figure(size = (800, 600))
    ax = Axis3(
        fig[1, 1],
        title = "Debye–Waller Factor  |⟨n|exp[iη(â + â†)]|m⟩|   (ν=$(round(secular_frequency, digits=2)))",
        xlabel = "n",
        ylabel = "m",
        zlabel = "|Ωₙₘ|",
    )
    ...
    for (i, n) in enumerate(n_vals), (j, m) in enumerate(m_vals)
        z = abs(ω[i, j])
        if z > 0
            poly!(
                ax,
                Rect3f(Vec3f(n - width/2, m - width/2, 0), Vec3f(width, width, z)),
                color = RGBA(0.2, 0.4, 0.8, 0.6),  # semi-transparent blue
                strokecolor = (:black, 0.1),         # subtle black edges
                shading = true
            )
        end
    end
    ...
    ax.viewmode = :fit
	#ax.projectiontype = :perspective
    fig
end


# ╔═╡ e050b0b9-9899-4ba3-84b1-ffc0c59ec91b
begin
@show ω
@show maximum(abs.(ω))
@show minimum(abs.(ω))
...
# ╔═╡ ee29ddf7-a414-484f-9af6-b82d830145f1
# begin
#     n_vals = 0:10
#     m_vals = 0:10
#     ω = rand(length(n_vals), length(m_vals))   # fake data so we KNOW z>0
...
manifest_format = "2.0"
project_hash = "5b8810eb7d5f37bfb17cdcb49a4de600ab5a7bdb"
